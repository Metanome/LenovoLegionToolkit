...
public partial class LampArrayDebugWindow : Window
{
    ...
    private async void OnLoaded(object sender, RoutedEventArgs e)
    {
        ...
        _effectSelect.SelectedIndex = 3;
        _effectTimer.Start();
    }

    private void InitializeKeyboardEvents()
    {
        var buttons = EnumerateKeys(_visualKeyboard);
        foreach (var button in buttons) button.Click += KeyButton_Click;
    ...
    private void InitializeKeyMapping()
    {
        ...
        foreach (var button in EnumerateKeys(_visualKeyboard))
        {
    ...
    private void KeyButton_Click(object sender, RoutedEventArgs e)
    {
        if (sender is LampArrayZoneControl button && _isCustomMode)
        {
            ...
            {
                Log(
                    $"Setting {indices.Length} Indices [{string.Join(",", indices)}] for Key {button.KeyCode} to selected color.");

                var colorDict = new Dictionary<int, Color>();
                foreach (var idx in indices) colorDict[idx] = _selectedColor;

                _controller.SetLampColors(colorDict);
                button.Color =
                    WpfColor.FromArgb(_selectedColor.A, _selectedColor.R, _selectedColor.G, _selectedColor.B);
            }
            else
            {
    ...
    private void EffectSelect_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        _isCustomMode = _effectSelect.SelectedIndex == 8;
        _customModePanel.Visibility = _isCustomMode ? Visibility.Visible : Visibility.Collapsed;

        if (!_isCustomMode)
        {
            ILampEffect effect = _effectSelect.SelectedIndex switch
            {
    ...
    private void BrightnessSlider_ValueChanged(object sender, RoutedEventArgs e)
    {
        if (sender is Slider slider)
        {
            ...
            if (_brightnessValue != null) _brightnessValue.Text = $"Brightness: {slider.Value:F0}%";
        }
    }
    ...
    private void SpeedSlider_ValueChanged(object sender, RoutedEventArgs e)
    {
        if (sender is Slider slider)
        {
            ...
            if (_speedValue != null) _speedValue.Text = $"Speed: {slider.Value:F0}%";
        }
    }

    private void SmoothTransition_Changed(object sender, RoutedEventArgs e)
    {
        if (_controller != null) _controller.SmoothTransition = _smoothTransitionCheckBox.IsChecked ?? true;
    }
    ...
    private void ClearCustomColors_Click(object sender, RoutedEventArgs e)
    {
        ...
        foreach (var key in EnumerateKeys(_visualKeyboard))
        {
            key.Color = null;
            key.IsChecked = false;
        }
    ...
    private void DumpLamps_Click(object sender, RoutedEventArgs e)
    {
        ...
        foreach (var detail in details) Log(detail);
        ...
        foreach (var item in lamps)
        {
            var i = item.Info;
            Log(
                $"Idx:{i.Index,-3} | Pos:({i.Position.X:F3},{i.Position.Y:F3},{i.Position.Z:F3}) | Purpose:{i.Purposes} | UpdLatency:{i.UpdateLatency}");
        }
        ...
        var controlIndices = lamps.Where(l => l.Info.Purposes.HasFlag(LampPurposes.Control)).Select(l => l.Info.Index)
            .ToList();
        Log($"Control ({controlIndices.Count}): {string.Join(",", controlIndices)}");

        var accentIndices = lamps.Where(l => l.Info.Purposes.HasFlag(LampPurposes.Accent)).Select(l => l.Info.Index)
            .ToList();
        Log($"Accent ({accentIndices.Count}): {string.Join(",", accentIndices)}");

        var brandingIndices = lamps.Where(l => l.Info.Purposes.HasFlag(LampPurposes.Branding)).Select(l => l.Info.Index)
            .ToList();
    ...
    private async void ProbeLamps_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            for (var index = 240; index <= 310; index++)
            {
                ...
                {
                    { index, Color.FromArgb(255, 255, 0, 0) }
                };
    ...
    private void Log(string msg)
    {
        ...
        {
            _logOutput.AppendText($"[{DateTime.Now:HH:mm:ss}] {msg}\n");
